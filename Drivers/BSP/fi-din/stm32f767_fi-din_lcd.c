...
uint8_t BSP_LCD_Init(void)
{
  ...
  hltdc.Init.AccumulatedActiveH = (LCD_PIXEL_HEIGHT + LCD_VSYNC + LCD_VBP - 1);
  hltdc.Init.AccumulatedActiveW = (LCD_PIXEL_WIDTH + LCD_HSYNC + LCD_HBP - 1);
  hltdc.Init.TotalHeigh = (LCD_PIXEL_HEIGHT + LCD_VSYNC + LCD_VBP + LCD_VFP - 1);
  hltdc.Init.TotalWidth = (LCD_PIXEL_WIDTH + LCD_HSYNC + LCD_HBP + LCD_HFP - 1);
...
void BSP_LCD_DrawPixel(uint16_t Xpos, uint16_t Ypos, uint32_t RGB_Code)
{
  ...
  if(hltdc.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_RGB565)
  { /* RGB565 format */
    uint16_t RGB565_Code = ((RGB_Code & 0xF80000) >> 8) | ((RGB_Code & 0xFC00) >> 5) | ((RGB_Code & 0xF8) >> 3);
    *(__IO uint16_t*) (hltdc.LayerCfg[ActiveLayer].FBStartAdress + (2*(Ypos*BSP_LCD_GetXSize() + Xpos))) = RGB565_Code;
  }
  else
  { /* ARGB8888 format */
...
__weak void BSP_LCD_ClockConfig(LTDC_HandleTypeDef *hltdc, void *Params)
{
  ...
  periph_clk_init_struct.PLLSAI.PLLSAIR = LCD_FREQ_DIV;
